<template>
  <div class="workflow">
    <el-row>
      ...
      <el-col :span="24">
        <div>
          <!-- 步骤条展示 active属性表示从哪一步开始 控制到了哪一步 -->
          <!-- align-center表示标题和描述可以居中 -->
          <el-card class="workflow-head-card" shadow="never" :body-style="{padding:'30px 10px 20px 10px'}">
            <el-steps :active="active" align-center finish-status="success">
              ...
              <el-step title="步骤2" description="填写 Deployment Workflow Workflow 表单"></el-step>
              <el-step title="步骤3" description="创建 Deployment Workflow Workflow"></el-step>
            </el-steps>
          </el-card>
        </div>
      </el-col>
          ...
          <el-card class="workflow-body-card" shadow="never" :body-style="{padding:'5px'}">
            <el-table
              ...
              <el-table-column align=center label="操作" width="200">
                <template v-slot="scope">
                  <el-button size="small" disabled style="border-radius:2px;" icon="Edit" type="primary" plain @click="getWorkflows(scope)">详情</el-button>
          ...
          <el-col :span="20">
            <el-form ref="createWorkflow" :rules="createWorkflowRules" :model="createWorkflow" label-width="80px">
              ...
              <el-form-item class="workflow-create-form" label="命名空间" prop="namespace">
                <el-select v-model="createWorkflow.namespace" filterable clearable placeholder="请选择命名空间" :no-data-text="'暂无数据'" default-first-option>
                  <el-option
                      v-for="(item, index) in namespaceList"
                      :key="index"
                      :label="item.metadata.name"
